...
{ 
	...
	{
	    // mre_fprintf(stderr, "strafe right\n");
	    side += sidemove[speed]; 
	}
	if (gamekeydown[key_left]) 
	{
	    //	mre_fprintf(stderr, "strafe left\n");
	    side -= sidemove[speed]; 
	}
	if (joyxmove > 0) 
	    side += sidemove[speed]; 
    ...
    {
	// mre_fprintf(stderr, "up\n");
	forward += forwardmove[speed]; 
    }
    if (gamekeydown[key_down]) 
    {
	// mre_fprintf(stderr, "down\n");
	forward -= forwardmove[speed]; 
    }
...
void G_DoLoadGame (void) 
{
    ...
    save_stream = mre_fopen(savename, "rb");
...
{ 
    ...
    save_stream = mre_fopen(temp_savegame_file, "wb");
    ...
    if (save_stream == NULL)
    {
        ...
        save_stream = mre_fopen(recovery_savegame_file, "wb");
        if (save_stream == NULL)
        {
    ...
    if (vanilla_savegame_limit && mre_ftell(save_stream) > SAVEGAMESIZE)
    {
...
{ 
    ...
    else
    {
        ...
        mre_printf(message, demoversion, G_VanillaVersionCode(),
