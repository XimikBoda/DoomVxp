...
static int I_MRE_StartSound(sfxinfo_t* sfxinfo, int channel, int vol, int sep)
{
    ...
    vm_bitstream_audio_start(bitstreams_handles[channel], &param);
...
static void I_MRE_StopSound(int handle)
{
    ...
    vm_bitstream_audio_stop(bitstreams_handles[handle]);
...
static boolean I_MRE_SoundIsPlaying(int handle)
{
    ...
    return true;//Mix_Playing(handle);
}
...
static void I_MRE_ShutdownSound(void)
{
    ...
    for (int i = 0; i < NUM_CHANNELS; ++i)
        vm_bitstream_audio_close(bitstreams_handles[i]);
...
static boolean I_MRE_InitSound(boolean _use_sfx_prefix)
{
    ...
    for (i = 0; i < NUM_CHANNELS; ++i)
        if (vm_bitstream_audio_open_pcm(&bitstreams_handles[i], &audio_type, bitstream_audio_result))
        {
