...
static boolean ReadSubstituteConfig(char* filename)
{
	...
	while (!feof(fs))
	{
		...
		if (error != NULL)
		{
			mre_fprintf(stderr, "%s:%i: Error: %s\n", filename, linenum, error);
		}
...
static void LoadSubstituteConfigs(void)
{
	...
	if (subst_music_len > 0)
	{
		mre_printf("Loaded %i music substitutions from config files.\n",
			subst_music_len);
	}
}
...
static void DumpSubstituteConfig(char* filename)
{
	...
	mre_fprintf(fs, "# Example %s substitute MIDI file.\n\n", PACKAGE_NAME);
	mre_fprintf(fs, "# SHA1 hash                              = filename\n");
	...
	for (lumpnum = 0; lumpnum < numlumps; ++lumpnum)
	{
		...
		{
			mre_fprintf(fs, "%02x", digest[h]);
		}

		mre_fprintf(fs, " = %s.ogg\n", name);
	}

	mre_fprintf(fs, "\n");
	mre_fclose(fs);

	mre_printf("Substitute MIDI config file written to %s.\n", filename);
	I_Quit();
}
...
static void I_MRE_ShutdownMusic(void)
{
...
static void timer(int tid);

// Initialize music subsystem
static boolean I_MRE_InitMusic(void)
{
	int i;
	...
	vm_bitstream_audio_open_pcm(&bitstream_handle, &audio_type, bitstream_audio_result);
...
static void UpdateMusicVolume(void)
{
	...
	{
		vol = (current_music_volume * 100) / 127;
	}
...
static void I_MRE_SetMusicVolume(int volume)
{
...
static void I_MRE_PlaySong(void* handle, boolean looping)
{
	int loops;
...
static void I_MRE_PauseSong(void)
{
	if (!music_initialized)
	{
		return;
...
static void I_MRE_ResumeSong(void)
{
	if (!music_initialized)
	{
		return;
...
static void I_MRE_StopSong(void)
{
	if (!music_initialized)
	{
		return;
...
static void I_MRE_UnRegisterSong(void* handle)
{
...
void midi_callback(VMINT handle, VMINT event) {}
void timer(int tid) {
	if (midi_player) {
		vm_bitstream_audio_buffer_status status;
		vm_bitstream_audio_get_buffer_status(bitstream_handle, &status);

		char buf[100 * 1024];
		int samples_count = adl_play(midi_player, status.free_buf_size / 2, buf);

		VMUINT writen = 0;
		vm_bitstream_audio_put_data(bitstream_handle, buf, samples_count * 2, &writen);
	}
}

static void* I_MRE_RegisterSong(void* data, int len)
{
	char* filename;
	//Mix_Music* music;
	void* music = NULL;
	...
	if (filename != NULL)
	{
		...
		if (music == NULL)
		{
			...
			// message.
			//mre_fprintf(stderr, "Failed to load substitute music file: %s: %s\n",
...

	{
		...
		midi_player = adl_init(11025);

		//adl_openData(midi_player, outbuf, outbuf_len);
		adl_openData(midi_player, data, len);


		vm_bitstream_audio_stop(bitstream_handle);
		...
		param.start_time = 0;
		param.volume = 100;
		vm_bitstream_audio_start(bitstream_handle, &param);
	}
	...
	if (music == NULL)
	{
		...
		//mre_fprintf(stderr, "Error loading midi: %s\n", Mix_GetError());
	}
...
// Is the song playing?
static boolean I_MRE_MusicIsPlaying(void)
{
	if (!music_initialized)
	{
		return false;
...
static void I_MRE_PollMusic(void)
{
	if (playing_substitute && file_metadata.valid)
	{
...
static snddevice_t music_mre_devices[] =
{
	SNDDEVICE_PAS,
	SNDDEVICE_GUS,
...
{
	music_mre_devices,
	arrlen(music_mre_devices),
	I_MRE_InitMusic,
	I_MRE_ShutdownMusic,
	I_MRE_SetMusicVolume,
	I_MRE_PauseSong,
	I_MRE_ResumeSong,
	I_MRE_RegisterSong,
	I_MRE_UnRegisterSong,
	I_MRE_PlaySong,
	I_MRE_StopSong,
	I_MRE_MusicIsPlaying,
	I_MRE_PollMusic,
};
